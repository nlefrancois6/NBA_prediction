...
data_all_years = pd.read_csv("pre-processedData_seasonAvg.csv")
...
#'teamAbbr','opptAbbr'
class_features = ['Season'] + away_features + home_features
...
#reg_features = ['V ML', 'H ML', 'Pred Score V', 'Pred Score H']
#v_score_model, h_score_model, profit_reg_model, testing_gains_reg = pf.layered_model_doubleReg_TrainTest(training_df, testing_df, class_features, output_label, class_params, reg_features, reg_label, reg_params, reg_threshold, plot_gains, fixed_wager, wager_pct)
...
if new_bets:
    account = 100
    fixed_wager = False
    current_data_df = validation_data_df[0:50]
